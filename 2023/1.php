...
// 1.2
$valid_digits = ["-*.....xhr......", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
...
foreach ($rows_array as $key => &$row) {
    foreach ($valid_digits as $key1 => $digit) {
        ...
        while (($lastPos = strpos($row, $digit, $lastPos)) !== false) {
            $allPositions[] = $lastPos;
            
            // changing letter to number
            $row[$lastPos + 1] = $key1;
